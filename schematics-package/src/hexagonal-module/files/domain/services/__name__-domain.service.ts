...
/**
 * <%= classify(moduleName) %> Domain Service
 *
...
@Injectable()
export class <%= classify(moduleName) %>DomainService {
  /**
   * Validate business rules for <%= classify(moduleName) %> entity
   *
   * This method encapsulates complex business rules that involve
   * multiple checks or external domain knowledge
   */
  validateBusinessRules(entity: <%= classify(moduleName) %>Entity): ValidationResult {
    const errors: string[] = [];
  ...
  /**
   * Business rule: Check if two <%= classify(moduleName) %> entities can be merged
   *
   * Example of a domain service method that coordinates between entities
   */
  canMerge(
    source: <%= classify(moduleName) %>Entity,
    target: <%= classify(moduleName) %>Entity,
  ): boolean {
    // Both must be active
  ...
  /**
   * Business rule: Merge two <%= classify(moduleName) %> entities
   *
  ...
  merge(
    ...
    if (!this.canMerge(source, target)) {
      throw new Error('Cannot merge these <%= classify(moduleName) %> entities');
    }
   ...
   */
  calculateScore(entity: <%= classify(moduleName) %>Entity): number {
    let score = 0;
   ...
   */
  isStale(entity: <%= classify(moduleName) %>Entity): boolean {
    const daysSinceUpdate = this.getDaysSince(entity.updatedAt);
   ...
   */
  private hasInvalidTimestamps(entity: <%= classify(moduleName) %>Entity): boolean {
    const now = new Date();
