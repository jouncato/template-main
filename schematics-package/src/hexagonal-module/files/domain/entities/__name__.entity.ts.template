...
export class <%= classify(moduleName) %>Entity {
  /**
  ...
  /**
   * Factory method to create a new <%= classify(moduleName) %> entity
  ...
  static create(props: {
    ...
    return new <%= classify(moduleName) %>Entity({
      id: props.id,
      name: props.name,
  ...
  static reconstitute(props: {
    ...
    return new <%= classify(moduleName) %>Entity(props);
  }
  ...
  private validate(): void {
    if (!this.id || this.id.trim().length === 0) {
      throw new Error('<%= classify(moduleName) %> entity must have a valid id');
    }

    if (!this.name || this.name.trim().length === 0) {
      throw new Error('<%= classify(moduleName) %> entity must have a valid name');
    }

    if (this.name.length > 255) {
      throw new Error('<%= classify(moduleName) %> name must not exceed 255 characters');
    }

    if (!this.createdAt) {
      throw new Error('<%= classify(moduleName) %> entity must have a createdAt timestamp');
    }

    if (!this.updatedAt) {
      throw new Error('<%= classify(moduleName) %> entity must have an updatedAt timestamp');
    }

    if (this.updatedAt < this.createdAt) {
      throw new Error('<%= classify(moduleName) %> updatedAt cannot be before createdAt');
    }
  }
   ...
   */
  activate(): <%= classify(moduleName) %>Entity {
    if (this.status === 'ACTIVE') {
      throw new Error('<%= classify(moduleName) %> is already active');
    }
   ...
   */
  deactivate(): <%= classify(moduleName) %>Entity {
    if (this.status === 'INACTIVE') {
      throw new Error('<%= classify(moduleName) %> is already inactive');
    }
   ...
   */
  equals(other: <%= classify(moduleName) %>Entity): boolean {
    if (!other) {
      return false;
    }
