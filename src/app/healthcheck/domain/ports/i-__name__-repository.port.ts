...
/**
 * <%= classify(moduleName) %> Repository Port (Outbound Port)
 *
...
export abstract class I<%= classify(moduleName) %>Repository {
  /**
   * Find a <%= classify(moduleName) %> entity by its unique identifier
   *
   * @param id - The unique identifier of the <%= classify(moduleName) %>
   * @returns Promise resolving to the entity if found, null otherwise
   */
  abstract findById(id: string): Promise<<%= classify(moduleName) %>Entity | null>;
   ...
   */
  abstract findAll(): Promise<<%= classify(moduleName) %>Entity[]>;
   ...
   */
  abstract findByStatus(status: string): Promise<<%= classify(moduleName) %>Entity[]>;

  /**
   * Find a <%= classify(moduleName) %> entity by name
   *
   * @param name - The name to search for
   * @returns Promise resolving to the entity if found, null otherwise
   */
  abstract findByName(name: string): Promise<<%= classify(moduleName) %>Entity | null>;
   ...
   */
  abstract save(entity: <%= classify(moduleName) %>Entity): Promise<<%= classify(moduleName) %>Entity>;
   ...
   */
  abstract update(entity: <%= classify(moduleName) %>Entity): Promise<<%= classify(moduleName) %>Entity>;

  /**
   * Delete a <%= classify(moduleName) %> entity by its identifier
   *
  ...
  /**
   * Check if a <%= classify(moduleName) %> entity exists by id
   *
   ...
   */
  abstract saveMany(entities: <%= classify(moduleName) %>Entity[]): Promise<<%= classify(moduleName) %>Entity[]>;
